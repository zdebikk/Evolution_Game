...
def main():
    ...
    while running:
        ...
        if not paused:
            ...
            # 1. Attack Logic
            r_to_p = (grid == 1) & (np.random.rand(COLS, ROWS) < neighbors_p * p_beat)
            p_to_s = (grid == 2) & (np.random.rand(COLS, ROWS) < neighbors_s * p_beat)
            s_to_r = (grid == 3) & (np.random.rand(COLS, ROWS) < neighbors_r * p_beat)
