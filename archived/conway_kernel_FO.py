...
# --- 2. Visuals ---
# Defined as float32 to allow smooth decimal subtraction
...
def main():
    ...
    # The visual display array uses floats to map the fade over time
    ...
    def update_caption():
        ...
        pygame.display.set_caption(f"[{status}] Mode {current_mode}: {modes[current_mode]} | Fade: {fade_status} (F) | Press 1-4 to switch")
    ...
    while running:
        ...
        if mouse_buttons[0] or mouse_buttons[2]:
            ...
            if 0 <= grid_x < COLS and 0 <= grid_y < ROWS:
                if mouse_buttons[0]: 
                    ...
                    display_rgb[grid_x, grid_y] = BG_COLOR # Instantly erase visual trail
        ...
        if fade_enabled:
            if not paused:
                # Subtract fade speed and clamp to background floor
                display_rgb -= FADE_SPEED
                display_rgb = np.maximum(display_rgb, BG_COLOR)
        else:
            # Wipe screen every frame if fading is disabled
            display_rgb[:, :] = BG_COLOR
