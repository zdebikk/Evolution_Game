...
def create_numpy_matrix(n: int) -> np.ndarray:
    size = 2 * n + 1
    matrix = np.ones((size, size), dtype=int)
    matrix[n, n] = 0
    return matrix

KERNEL = np.array(create_numpy_matrix(COLS // 2-1))
...
def main():
    ...
    p_die = 0.00        # (E/D) Rate at which cells spontaneously die
    varepsilon = 0.000  # (T/G) Spontaneous generation into empty cells
    p_beat_r = 0.7  # Rock invades Scissors faster (advantaged offensively)
    p_beat_p = 0.9  # Paper invades Rock at baseline
    p_beat_s = 0.9  # Scissors invades Paper at baseline
